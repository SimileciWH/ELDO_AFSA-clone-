...
shiftFreq = 6;%觅食行为和{群聚，追尾}行为的切换频率
countMin = 2;%记录最优解个数.
rightInf = sqrt(2);
...
%================================================================================================================
    [Hmin(j),index(j)] = min(H);%找到Hmin对应的位置.存在index中  
    %要更新ppValue的值
    ppValue =  position(j,:);
    %更新最小值到arrayValue中去
    if Hmin(j) < DistMin
        DistMin = Hmin(j);
        arrayValue(countMin) = position(j,index(j));%把每一步中最优的解记录下来  
    else
        DistMin = DistMin;
...
% %%
% %经验参考修正问题1
    if distance(n,arrayValue(countMin),arrayValue(countMin-1)) > rightInf
%        Barrier = [Barrier arrayValue(countMin-1)];
       for i = 1:1:f%清空需要清空的数据
           position(j-i+1,:) = [];
           arrayValue(countMin-i+1) = [];            
       end
       j = j-f-1;
       countMin = countMin-f;
       ppValue =  position(j-1,:);
       H =GrideAF_foodconsistence(n,ppValue,goal);
       disp("修正!!!")
    end
